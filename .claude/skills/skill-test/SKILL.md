...
- `category [name]` → run category-specific rubric from `.claude/testing/quality-rubric.md`
...
Look up the spec path from `.claude/testing/catalog.yaml` — use the
...
Date: [date]
Spec: .claude/testing/skills/[category]/[name].md
...
"May I write these results to `.claude/testing/results/skill-test-spec-[name]-[date].md`
and update `.claude/testing/catalog.yaml`?"

If yes:
- Write results file to `.claude/testing/results/`
- Update the skill's entry in `.claude/testing/catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `.claude/skills/[name]/SKILL.md`.
Look up `category:` field in `.claude/testing/catalog.yaml`.
...
Read `.claude/testing/quality-rubric.md`.
...
"May I update `.claude/testing/catalog.yaml` to record this category check
...
Read `.claude/testing/catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).
...
Also read the `agents:` section from `.claude/testing/catalog.yaml` to get the
complete list of agents.

### Step 3 — Build Skill Coverage Table

For each skill:
- Check if a spec file exists (use the `spec:` path from catalog, or glob `.claude/testing/skills/*/[name].md`)
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `.claude/testing/agents/*/[name].md`)
...
- After `spec [name]` PASS: "Update `.claude/testing/catalog.yaml` to record this
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `.claude/testing/templates/skill-test-spec.md` to create new specs."
